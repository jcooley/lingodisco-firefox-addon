...
let translateWidget =  require("widget").Widget({
    id: 'ocuile-aistrigh',
    label: "Translate",
    contentURL: data.url("disco-white-redcentre.png"),
...
translation_scraper_panel.port.on('update_selection', function(source, translation){
    if (!translation_on){    
	console.log("trying to update selection when translation_on is " + translation_on);
	return;
    }
...
function update_content_script_state(){
    translateWidget.contentURL = translation_on?data.url("disco-white-redpause.png"):data.url("disco-white-redcentre.png");
    if(translation_on){
