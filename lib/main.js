...
var control_panel = require("sdk/panel").Panel({
    ...
    contentScriptFile: [self.data.url("jquery-1.7.2.js"), 
			self.data.url('onoffpanel.js')
		       ],
  onHide: handleHide
});
...
var translateWidget =  ActionButton({
    id: "ocuile-aistrigh",
    label: "Translate",
    icon: self.data.url("disco-white.png"),
...
function update_content_script_state(){
    translateWidget.icon = translation_on?self.data.url("disco-white-redsquarecentre.png"):self.data.url("disco-white.png");
    if(translation_on){
