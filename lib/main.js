...
function reset_scraper(){
	...
	console.log("resetting scraper "+ translation_scraper_panel.contentURL +" -> "+ prefs.translation_url); 
	translation_scraper_panel.contentURL = prefs.translation_url;
    }else{
	console.log("-not resetting scraper  translation_scraper_panel.contentURL  ==  prefs.translation_url"); 
    }
...
translation_scraper_panel.port.on('update_selection', function(source, translation){
    RESET_WORKING = true;
    if (!translation_on){    
	console.log("trying to update selection when translation is " + translation_on);
	return;
    }
